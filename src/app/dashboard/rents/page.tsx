...
const Rent = () => {
  ...
  const columns = [
    {
      ...
      dataIndex: "vehicle",
      key: "_id",
      render: (vehicle: IVehicle, item: IRent) => (
        <div className="text-center">
          {vehicle.status === StatusEnum.AVAILABLE ? (
            <CheckOutlined
              onClick={() => rentVehicle(item._id, item)}
              style={{ color: "#6582EB" }}
            />
          ) : (
            <RollbackOutlined
    ...
    },
    {
      ...
      render: (payment: string) => (
        <p>${payment ? Intl.NumberFormat().format(Number(payment)) : 0}</p>
      ),
    },
    {
      title: "Total",
      ...
      key: "total",
      render: (total: string) => (
        <p>${total ? Intl.NumberFormat().format(Number(total)) : 0}</p>
      ),
    },
    {
    ...
    },
    {
      ...
      dataIndex: "timestamp",
      key: "timestamp",
      render: (timestamp: string, item: IRent, idx: number) => (
  ...
  const rentVehicle = async (id: string, item: IRent) => {
    const status =
      item.vehicle.status === StatusEnum.AVAILABLE
        ? StatusEnum.RENTED
  ...
  const loadDropdrowns = async () => {
    ...
    const clientOptions = clients.map((el: IClient) => ({
      value: JSON.stringify(el),
      label: el.name,
    }));
    const vehicleOptions = vehicles.map((el: IVehicle) => ({
      value: JSON.stringify(el),
      label: el.brand,
    }));
  ...
  return (
    <div className="rents-content">
      ...
      <Drawer
        ...
        onClose={onClose}
        title={
          action === ActionEnum.ADD ? "Agregar Alquiler" : "Actualizar Alquiler"
        }
      >
